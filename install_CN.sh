...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        ...
        wget -N --no-check-certificate -O /usr/local/x-ui-linux-${arch}.tar.gz https://5th.pp.ua/https://github.com/sing-web/x-ui/releases/download/${last_version}/x-ui-linux-${arch}.tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://5th.pp.ua/https://github.com/sing-web/x-ui/releases/download/${last_version}/x-ui-linux-${arch}.tar.gz"
        echo -e "开始安装 x-ui $1"
    ...
    wget --no-check-certificate -O /usr/bin/x-ui https://5th.pp.ua/https://raw.githubusercontent.com/sing-web/x-ui/main/x-ui_CN.sh
